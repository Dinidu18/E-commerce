...
const SERVICE_NAME = 'Reviews & Ratings Service';
...
const swaggerOptions = {
  definition: {
    ...
    info: {
      ...
      version: '1.0.0',
      description: 'REST API for managing product reviews and customer ratings.',
      contact: { name: 'E-Commerce Dev Team', email: 'dev@ecommerce.com' },
    },
    servers: [
      { url: `http://localhost:${PORT}`, description: 'Local Development Server' },
      { url: 'http://localhost:8080/reviews', description: 'Via API Gateway' },
    ],
    tags: [
      { name: 'Reviews', description: 'Review & rating management endpoints' },
...
const swaggerUiOptions = {
  explorer: true,
  customSiteTitle: '⭐ Reviews - API Documentation',
  customCss: `
    :root { --bg-dark: #0f0f1e; --text-primary: #f5f5f7; --text-secondary: #a0a0a6; --accent: #f2cc60; --border-color: rgba(255, 255, 255, 0.08); }
    ...
    .swagger-ui .btn { background: var(--accent); color: #0f0f1e; border: none; border-radius: 8px; font-weight: 600; }
    .swagger-ui .btn:hover { background: #f4d666; }
    ...
    .swagger-ui input:focus, .swagger-ui select:focus, .swagger-ui textarea:focus { border-color: var(--accent); box-shadow: 0 0 0 3px rgba(242, 204, 96, 0.1); }
  `,
};
...
app.use((req, res) => {
  res.status(404).json({ success: false, message: `Cannot ${req.method} ${req.originalUrl}`, hint: 'Try GET /api/reviews or visit /api-docs' });
});
