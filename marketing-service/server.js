...
const SERVICE_NAME = 'Marketing & Promotion Service';
...
const swaggerOptions = {
  definition: {
    ...
    info: {
      ...
      version: '1.0.0',
      description: 'REST API for managing promotions, discount codes, and marketing campaigns.',
      contact: { name: 'E-Commerce Dev Team', email: 'dev@ecommerce.com' },
    },
    servers: [
      { url: `http://localhost:${PORT}`, description: 'Local Development Server' },
      { url: 'http://localhost:8080/marketing', description: 'Via API Gateway' },
    ],
    tags: [
      { name: 'Promotions', description: 'Promotion & discount management endpoints' },
...
const swaggerUiOptions = {
  explorer: true,
  customSiteTitle: '🎁 Promotions - API Documentation',
  customCss: `
    :root { --bg-dark: #0f0f1e; --text-primary: #f5f5f7; --text-secondary: #a0a0a6; --accent: #ffa657; --border-color: rgba(255, 255, 255, 0.08); }
    ...
    .swagger-ui .btn:hover { background: #ff924d; }
    ...
    .swagger-ui input:focus, .swagger-ui select:focus, .swagger-ui textarea:focus { border-color: var(--accent); box-shadow: 0 0 0 3px rgba(255, 166, 87, 0.1); }
  `,
};
...
app.use((req, res) => {
  res.status(404).json({ success: false, message: `Cannot ${req.method} ${req.originalUrl}`, hint: 'Try GET /api/promotions or visit /api-docs' });
});
