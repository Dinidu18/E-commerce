...
const SERVICE_NAME = 'Finance Management Service';
...
const swaggerOptions = {
  definition: {
    ...
    info: {
      ...
      version: '1.0.0',
      description: 'REST API for managing financial transactions and invoices.',
      contact: { name: 'E-Commerce Dev Team', email: 'dev@ecommerce.com' },
    },
    servers: [
      { url: `http://localhost:${PORT}`, description: 'Local Development Server' },
      { url: 'http://localhost:8080/finance', description: 'Via API Gateway' },
    ],
    tags: [
...
const swaggerUiOptions = {
  ...
  customCss: `
    :root { --bg-dark: #0f0f1e; --text-primary: #f5f5f7; --text-secondary: #a0a0a6; --accent: #da7aff; --border-color: rgba(255, 255, 255, 0.08); }
    ...
    .swagger-ui .btn:hover { background: #c566ff; }
    ...
    .swagger-ui input:focus, .swagger-ui select:focus, .swagger-ui textarea:focus { border-color: var(--accent); box-shadow: 0 0 0 3px rgba(218, 122, 255, 0.1); }
  `,
};
...
app.use((req, res) => {
  res.status(404).json({ success: false, message: `Cannot ${req.method} ${req.originalUrl}`, hint: 'Try GET /api/transactions or visit /api-docs' });
});
